...
//			'volunteer_nick_name' 	=> 'Pendamping',
		...
		'custom' 	=> array(
			...
			'review_mc' 	=> '"<div class=\"rating-readonly\" data-rating=\"{$item["review"]}\"></div><span style=\"display: none\">{$item["review"]}</span>"',
		),
	);
...
//		$this->list_field['custom']['volunteer_nick_name'] = '"<a href=\"' . $this->group . 'report/volunteer/detail/{$item["volunteer_id"]}\">{$item["volunteer_nick_name"]}</a>"';
        $this->list_field['custom']['name'] = '"<a href=\"' . $this->group . 'report/pendampingan/detail/{$item["student_id"]}\">{$item["name"]}</a>"';
        $this->list_field['custom']['volunteer_name'] = '"<a href=\"' . $this->group . 'report/volunteer/detail/{$item["volunteer_id"]}\">{$item["volunteer_name"]}</a>"';
...
//		$this->aquery->set_meta_data('data_star', $data_star);
	...
	protected function filter()
	{
        ...
        if (isset($get_data['tgl_awal']) === true && $get_data['tgl_awal']) {
            $this->model->where('date >= "' . $get_data['tgl_awal'] . '"');
        }
        if (isset($get_data['tgl_akhir']) === true && $get_data['tgl_akhir']) {
