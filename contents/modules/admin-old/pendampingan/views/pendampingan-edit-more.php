<div class="content max-md-container">
		...
		<form class="card" method="post" enctype="multipart/form-data" <?php if (isset($form_action) && $form_action) echo 'action="' . $form_action . '"'; ?>>
			<div class="card-header d-flex justify-content-between align-items-center">
				<h4 class="card-title">
					...
					<?php echo 'Edit Jamak ' . ucwords($module_name); ?>
				</h4>
			</div>
			<?php echo $errors; ?>
			<div class="card-body">
				<?php if ($mode_add === true): ?>
					...
					<div class="form-group row">
						...
						<div class="col-sm-8">
							<span class="form-control bg-light"><?php echo $day[(empty($data['day']) ? date("N", strtotime($date)): $data['day'])].', '.date_html($date); ?> <small class="text-muted">(<?php echo time_html($data['start_at']) . ' - ' . time_html($data['end_at']); ?>)</small></span>
						</div>
					</div>
				<?php else: ?>
					<div class="form-group row">
						<label class="col-sm-4 col-form-label">Nama Mahasiswa</label>
						<div class="col-sm-8">
							<span class="form-control bg-light"><a href="<?php echo $group.'mahasiswa/edit/'.$post['user_id'] ?>"><?php echo $post['name']; ?></a><small class="text-muted"> (<?php echo $post['nick_name']; ?>)</small></span>
						</div>
					</div>
					<div class="form-group row">
						<label class="col-sm-4 col-form-label">Waktu Pendampingan</label>
						<div class="col-sm-8">
							<span class="form-control bg-light"><?php echo $day[(empty($post['day']) ? date("N", strtotime($post['date'])) : $post['day'])].', '.date_html($post['date']); ?> <small class="text-muted">(<?php echo time_html($post['start_at']) . ' - ' . time_html($post['end_at']); ?>)</small></span>
						</div>
					</div>
				<?php endif ?>
				<hr>
