						...
						<tr>
							...
							<?php if (isset($action_url['duplicate']) || isset($action_url['edit']) || isset($action_url['delete'])): ?>
								<td class="text-center text-nowrap">
									<?php if ($item["approver_id"] === null): ?>
										<a href="<?php echo $group.$module_main.'/approve/'.$item["register_id"]; ?>" class="text-success"><i class="fas mr-1 fa-check"></i></a> &nbsp;
									<?php endif ?>
                                    <?php if ($item["approver_id"] === null && !empty($item['rev_at'])){ ?>
                                        <a href="<?php echo $group.'jadwal/'.$item['capability'].'/revisi/'.$item["student_id"]."/".($item["rev"]+1); ?>"><i class="fa mr-1 fa-calendar-alt"></i></a>
                                    <?php } else { ?>
                                        <a href="<?php echo $group.'jadwal/'.$item['capability'].'/detail/'.$item["student_id"]; ?>"><i class="fa mr-1 fa-calendar-alt"></i></a>
                                    <?php } ?>
