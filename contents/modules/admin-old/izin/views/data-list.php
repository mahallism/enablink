						...
						<tr>
							...
							<?php if (isset($action_url['duplicate']) || isset($action_url['edit']) || isset($action_url['delete'])): ?>
								<td class="text-center text-nowrap">
									<?php if ($item['student'] === 'Volunteer' && $item['status'] == 'menunggu konfirmasi'): ?>
										<a href="<?php echo $group.'pendampingan/change/'.$item['accompaniment_id'].'?permit='.$item['permit_id'] ?>"><i class="fas fa-user-check mr-1"></i>Gantikan</a>
									<?php endif ?>
									<?php if ($item['student'] === 'Difabel' && $item['status'] == 'menunggu konfirmasi'): ?>
                                        <a href="<?php echo $group.'pendampingan/cancel/'.$item['accompaniment_id'].'?permit='.$item['permit_id'] ?>"><i class="fas fa-calendar-times mr-1"></i> Verifikasi izin</a>
                                    <?php endif ?>
