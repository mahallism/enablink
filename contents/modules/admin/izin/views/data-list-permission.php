						...
						<tr>
							...
							<?php if (isset($action_url['duplicate']) || isset($action_url['edit']) || isset($action_url['delete'])): ?>
								<td class="text-center text-nowrap">
									<?php if (strtolower($item['student']) === 'volunteer' && $item['status'] == 'menunggu konfirmasi'): ?>
										<a href="<?php echo $group.'izin/permission/gen_change?permission='.$item['permission_id'] ?>" onclick="return confirm('Anda yakin mengganti semua pendampingan mahasiswa ini ditanggal tsb?')"><i class="fas fa-user-check mr-1"></i>Atur pergantian jadwal</a>
									<?php endif ?>
                                    <?php if (strtolower($item['student']) === 'difabel' && $item['status'] == 'menunggu konfirmasi'): ?>
                                        <a href="<?php echo $group.'izin/permission/cancel?permission='.$item['permission_id'] ?>"><i class="fas fa-calendar-times mr-1"></i> Verifikasi izin</a>
                                    <?php endif ?>
									...
									<?php endif ?>
									<?php if (isset($action_url['delete']['url']) && $item['status'] == 'menunggu konfirmasi'): ?>
