...
class Difabel extends App_Controller {
	...
	protected $module_main = 'active/difabel';
	protected $module_name = 'mahasiswa Difabel';

	protected $capability_mahasiswa = 'difabel';
	...
	public function approve($id = false)
	{
		...
		if (!$data) {
            ...
            $activeStudentUpdate = array(
                'approver_id' => current_user_session('user_id'),
                'last_rev_at' => $data['rev_at'],
                'last_approve_at' => date("Y-m-d H:i:s"),
                'rev_at' => null,
            );
            if(!empty($data['rev_at'])) {
                $activeStudentUpdate['rev'] = $data['rev'] + 1;
                $this->load->model($this->group . 'jadwal/jadwal_difabel_model');
                if($data['rev'] == "0"){
                    $this->jadwal_difabel_model->backup_jadwal_active_student(array(
                        "student_id"=>$data['student_id'],
                        "semester_id"=>$data['semester_id']
                    ));
                }
                $this->jadwal_difabel_model->softdelete_jadwal_active_student(array(
                    "student_id"=>$data['student_id'],
                    "semester_id"=>$data['semester_id'],
					"rev"=>$data['rev']
                ));
                $this->jadwal_difabel_model->set_jadwal_rev_active_student(array(
                    "student_id"=>$data['student_id'],
                    "semester_id"=>$data['semester_id'],
                    "rev"=>$data['rev'] + 1
                ));
            }
            $this->model->update($id, $activeStudentUpdate);
	...
	public function input($id = false)
	{
		...
		$this->list_field['custom']['action'] .= '."<input type=\"checkbox\" name=\"" . ($item["register_id"] === null ? "new" : "approve") . "[]\" value=\"{$item["student_id"]}\">"';
