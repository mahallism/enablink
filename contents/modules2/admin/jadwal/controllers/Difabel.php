...
class Difabel extends App_Controller
{
	...
	protected $module_name = 'jadwal mahasiswa';
	...
	public function index($id = false)
	{
		...
		$custom_field_name = '"<a href=\"' . $this->group . 'mahasiswa/edit/{$item["user_id"]}\">{$item["name"]}</a> <small class=\"text-muted\">({$item["nick_name"]})</small>"';
		$custom_field_name .= ' . "<div><small>{$item["faculty_name"]} - {$item["majors_name"]}</small></div>"';
		$custom_field_name .= ' . "<a href=\"' . $this->group . $this->module_main . '/input/{$item["student_id"]}\" class=\"btn btn-sm btn-primary my-1 mr-1\"><i class=\"fa fa-plus-circle fa-fw mr-1\"></i>Input Jadwal</a>"';
		$custom_field_name .= ' . "<a href=\"' . $this->group . $this->module_main . '/detail/{$item["student_id"]}\" class=\"btn btn-sm btn-primary my-1\"><i class=\"fa fa-eye fa-fw mr-1\"></i>Lihat Jadwal</a>"';
	...
	public function detail($id = false)
	{
		...
		$data = $this->model->where('student_id', $id)->where('semester_id', $this->current_semester)->gets_view();
